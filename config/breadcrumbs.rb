...
crumb :order do |model|
  link model.new_record? ? t('views.breadcrumbs.orders.new') : model.campaign_name, model
  parent :orders
end

crumb :sub_total_settings do
  link t('views.breadcrumbs.sub_total_settings.index'), sub_total_settings_path
end

crumb :sub_total_setting do |model|
  link model.new_record? ? t('views.breadcrumbs.sub_total_settings.new') : model.name, model
  parent :sub_total_settings
end
...
crumb :country_costs do |model|
  link t('views.breadcrumbs.country_costs.index'), country_costs_path
end

crumb :country_cost do |model|
  link model.new_record? ? t('views.breadcrumbs.country_costs.new') : model.country_name, model
  parent :country_costs
end

crumb :daily_est_imps do |model|
  link t('views.breadcrumbs.daily_est_imps.index'), daily_est_imps_path
end

crumb :daily_est_imp do |model|
  link model.new_record? ? t('views.breadcrumbs.daily_est_imps.new') : model.country_name, model
  parent :daily_est_imps
end

crumb :banner_sizes do |model|
  link t('views.breadcrumbs.banner_sizes.index'), daily_est_imps_path
end

crumb :banner_size do |model|
  link model.new_record? ? t('views.breadcrumbs.banner_sizes.new') : model.name, model
  parent :banner_sizes
end

crumb :densities do |model|
  link t('views.breadcrumbs.densities.index'), densities_path
end

crumb :density do |model|
  link model.new_record? ? t('views.breadcrumbs.density.new') : model.name, model
  parent :densities
end
