class Reports::Helpers::AgencyFormula
  ...
  def total_price(campaign_type, views, clicks, creative_total)
    if campaign_type == :CPM
      per_view_price = creative_total.total_price / creative_total.impressions
      (per_view_price * views).round(3)
    else
      per_click_price = creative_total.total_price / creative_total.clicks
      (per_click_price * clicks).round(3)
    end
