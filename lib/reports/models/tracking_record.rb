module Reports::Models::TrackingRecord
  ...
  def hash_detail
    ...
    unless @unit_price.nil?
      json[:unit_price] = number_with_delimiter(@unit_price)
    end
    unless @total_price.nil?
      json[:total_price] = number_with_delimiter(@total_price)
    end
  ...
  def init_detail(detail, campaign = nil)
    ...
    if detail.respond_to?(:unit_price)
      @unit_price = detail.unit_price
    end

    if detail.respond_to?(:total_price)
      @total_price = detail.total_price.round(2)
    end
